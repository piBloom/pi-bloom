...
- Started the design-system migration with production `ds-button` and `ds-avatar` web components for static and dynamic controls across the topbar, sidebars, status banner, modals, chat toolbar, model picker, image previews, session list, and telemetry tabs.
