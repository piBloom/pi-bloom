...
export function seedBlueprints(bloomDir: string, packageDir: string): void {
	...
	const personaDir = fs.existsSync(path.join(packageDir, "core", "persona"))
		? path.join(packageDir, "core", "persona")
		: path.join(packageDir, "persona");
	const skillsDir = fs.existsSync(path.join(packageDir, "core", "skills"))
		? path.join(packageDir, "core", "skills")
